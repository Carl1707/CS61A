...
class FreeChecking(Account):
    ...
    def __init__(self,holder):
        ...
        self.free_times = FreeChecking.free_withdrawals

    def withdraw(self, amount):
        if self.free_times == 0:
            if self.balance >= amount + FreeChecking.withdraw_fee:
    ...
    #Another version that doesn't require __init__
    #def withdraw(self, amount):
...
def add_d_leaves(t, v):
    ...
    """
    #实在无法追踪某个参数，就使用辅助函数
    def add_leaves_helper(tree_node, value, depth):
        # 步骤 1: 对每个分支进行递归，并增加深度
        for branch in tree_node.branches:
            # 只有当分支本身也是一个Tree对象时才进行递归
            # (这可以防止我们对刚刚添加的叶子进行递归)
            if isinstance(branch, Tree):
                add_leaves_helper(branch, value, depth + 1)
        # 步骤 2: 为当前节点添加 d 个叶子
        # 深度为 d，就添加 d 个新的 Tree(value) 到 branches
        for _ in range(depth):
            tree_node.branches.append(Tree(value))
    # 从根节点开始调用，初始深度为 0
...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    t1, t2 = t.branches[0], t.branches[1]
    t.branches.clear()
    if t1.label <= t2.label:
        t.branches.append(t1)
        return prune_min(t1)
    else:
        t.branches.append(t2)
        return prune_min(t2)
