...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    i , max , max_rolls = 1 , 0 , 1
    while i <= 10 :
        # make_averaged 返回一个函数
        cur = make_averaged(roll_dice , times_called) (i , dice )
        if cur > max :
            # 这两次调用可能返回值不同，只能调用一次而用变量保存
            max = cur
            max_rolls = i
        i += 1
    return max_rolls
...
AVERAGE_6_ROLL_SCORE = make_averaged(roll_dice, 1000)(6, six_sided)
...
BENCHMARK_SCORE = make_averaged(roll_dice, 1000)(DEFAULT_ROLL, six_sided)

def final_strategy(score, opponent_score):

    if sus_update(0, score, opponent_score) >= GOAL:
        return 0
    if score - opponent_score > LEAD_MARGIN:
        base_num_rolls = SAFE_ROLL
    else:
        base_num_rolls = DEFAULT_ROLL

    return sus_strategy(score, opponent_score, threshold=BENCHMARK_SCORE, num_rolls=base_num_rolls)
