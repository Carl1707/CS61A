...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        #仍然是同样问题，应该scheme_eval expressions.rest.first才可以
        ...
        new_expression = Pair(formals, body)
        #创造一个新的Pair用于do_lambda_form的评估
        result = do_lambda_form(new_expression, env)
        env.define(func_name, result)
        return func_name
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    result = LambdaProcedure(formals, expressions.rest, env)
    return result
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        #应该evaluate bindings.first.rest.first，注意最后要有first
        bindings = bindings.rest
    validate_formals(names)
    child = env.make_child_frame(names, vals)
    return child
    # END PROBLEM 14
